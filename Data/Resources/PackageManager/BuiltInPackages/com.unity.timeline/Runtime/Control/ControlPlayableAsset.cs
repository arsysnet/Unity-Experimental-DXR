...
namespace UnityEngine.Timeline
{
    ...
    public class ControlPlayableAsset : PlayableAsset, IPropertyPreview, ITimelineClipAsset, IClipInitializer, IDirectorDriver
    {
        ...
        void UpdateDurationAndLoopFlag(IList<PlayableDirector> directors, IList<ParticleSystem> particleSystems)
        {
        ...
        void IClipInitializer.OnCreate(TimelineClip newClip, TrackAsset track, IExposedPropertyTable table)
        {
            ...
            if (table != null)
                sourceObject = sourceGameObject.Resolve(table);

            if (sourceObject == null && prefabGameObject != null)
                sourceObject = prefabGameObject;

            if (sourceObject)
            {
                var directors = GetComponent<PlayableDirector>(sourceObject);
                var particleSystems = GetComponent<ParticleSystem>(sourceObject);

                // update the duration and loop values (used for UI purposes) here
                // so they are tied to the latest gameObject bound
                UpdateDurationAndLoopFlag(directors, particleSystems);

                newClip.displayName = sourceObject.name;
            }
        }
        ...
        IList<PlayableDirector> IDirectorDriver.GetDrivenDirectors(IExposedPropertyTable resolver)
        {
            ...
            if (!updateDirector || prefabGameObject != null || resolver == null)
                return new List<PlayableDirector>(0);

            var go = sourceGameObject.Resolve(resolver);

            if (go == null)
                return new List<PlayableDirector>(0);

            var filteredDirectors = new List<PlayableDirector>();

            foreach (var director in GetComponent<PlayableDirector>(go))
            {
