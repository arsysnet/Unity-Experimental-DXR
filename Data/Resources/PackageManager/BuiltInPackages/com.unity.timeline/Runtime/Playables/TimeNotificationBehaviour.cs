...
namespace UnityEngine.Timeline
{
    ...
    public class TimeNotificationBehaviour : PlayableBehaviour
    {
        ...
        public override void PrepareFrame(Playable playable, FrameData info)
        {
            ...
            {
                var duration = playable.GetDuration();
                TriggerNotificationsInRange(m_PreviousTime, duration, info, playable, false);
            ...
            {
                var e = m_Notifications[i];
                if (e.notificationFired && CanRestoreNotification(e, info, currentTime))
                {
        ...
        static bool CanRestoreNotification(NotificationEntry e, FrameData info, double currentTime)
        {
            if (e.triggerOnce)
                return false;

            //case 1111595: restore the notification if the time is manually set before it
            if (info.seekOccurred && currentTime <= e.time)
                return true;

            return info.timeLooped;
        }
