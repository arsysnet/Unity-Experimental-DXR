...
namespace UnityEditor.Timeline
{
    class TimelineTreeView : ITreeViewGUI
    {
        ...
        {
            SequencerContextMenu.ShowNewTracksContextMenu(null, null, m_State);
            Event.current.Use();
        }
        ...
        protected virtual Vector2 GetSizeOfRow(TreeViewItem item)
        {
            ...
            float height = m_State.trackHeight;
            if (item.hasChildren && m_TreeView.data.IsExpanded(item))
            {
                height = Mathf.Min(m_State.trackHeight, kMinTrackHeight);
            }
        ...
        public virtual void OnRowGUI(Rect rowRect, TreeViewItem item, int row, bool selected, bool focused)
        {
            ...
            if (Event.current.type == EventType.Repaint)
            {
                ...
                m_State.spacePartitioner.AddBounds(new TrackHeaderBounds(sqvi, headerRect));
        ...
        private void DrawFoldout(TreeViewItem item, Rect foldoutRect, float indent)
        {
            ...
            if (showFoldout)
            {
                ...
                float foldoutIconHeight = DirectorStyles.Instance.foldout.normal.background.height;
