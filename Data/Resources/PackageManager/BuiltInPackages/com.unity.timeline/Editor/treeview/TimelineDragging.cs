...
namespace UnityEditor
{
    class TimelineDragging : TreeViewDragging
    {
        ...
        public static bool ResolveType(IEnumerable<System.Type> types, Action<Type> onComplete, string formatString)
        {
            ...
            var builtInTypes = types.Where(x => x.Assembly == typeof(TimelineAsset).Assembly).OrderBy(TypeUtility.GetDisplayName).ToArray();
            var customTypes = types.Where(x => x.Assembly != typeof(TimelineAsset).Assembly).OrderBy(TypeUtility.GetDisplayName).ToArray();
            ...
            if (perform)
            {
                System.Action<Type> onResolve = trackType =>
                {
                    foreach (var obj in objectsBeingDropped)
                    {
                        ...
                        {
                            var newTrack = timeline.CreateTrack(trackType, targetTrack, string.Empty);
                            ...
                            if (trackType == typeof(ActivationTrack))
                                newTrack.CreateClip(0).displayName = ActivationTrackDrawer.Styles.ClipText.text;
                            if (targetTrack != null)
                                targetTrack.SetCollapsed(false);
                        }
                    }
        ...
        public static DragAndDropVisualMode HandleClipPanePlayableAssetDragAndDrop(IEnumerable<IPlayableAsset> assetsBeingDropped, TrackAsset targetTrack, bool perform, TimelineAsset timeline, TrackAsset parent, PlayableDirector director, double candidateTime, TypeResolver typeResolver)
        {
            ...
            if (perform)
            {
                ...
                {
                    if (targetTrack == null)
                        targetTrack = timeline.CreateTrack(t, parent, string.Empty);
