    ...
    class TimeAreaItem : Control
    {
        ...
        float widgetHeight
        {
            get
            {
                ...
                if (height < 1.0f)
                    return m_Style.normal.background.height;
        ...
        float widgetWidth
        {
            get
            {
                ...
                if (width < 1.0f)
                    return m_Style.normal.background.width;
        ...
        public TimeAreaItem(GUIStyle style, Action<double> onDrag)
        {
            ...
            var scrub = new Scrub((evt, state) => state.timeAreaRect.Contains(evt.mousePosition) && bounds.Contains(evt.mousePosition), onDrag, () => { showTooltip = false; });
        ...
        public void Draw(Rect rect, WindowState state, double time)
        {
            ...
            using (new GUIViewportScope(clipRect))
            {
                ...
                float y = state.timeAreaRect.yMax - DirectorStyles.kDurationGuiThickness;

                Vector3 p1 = new Vector3(windowCoordinate.x, y, 0.0f);
                Vector3 p2 = new Vector3(windowCoordinate.x, y + Mathf.Min(rect.height + 3, state.windowHeight), 0.0f);

                if (drawLine)
                {
                    Rect lineRect = Rect.MinMaxRect(p1.x - 0.5f, p1.y, p2.x + 0.5f, p2.y);
