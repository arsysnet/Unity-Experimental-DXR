...
namespace UnityEditor.Timeline
{
    ...
    class ClipInspector : Editor
    {
        internal static class Styles
        {
            ...
            public static readonly GUIContent TimelineClipFG = EditorGUIUtility.IconContent("TimelineClipFG");
            public static readonly GUIContent TimelineClipBG = EditorGUIUtility.IconContent("TimelineClipBG");
        }
        ...
        class EditorClipSelection
        {
            ...
            public SerializedObject playableAssetObject { get; }
            ...
            public EditorClipSelection(EditorClip anEditorClip)
            {
                ...
                if (playableAssetProperty != null)
                {
                    ...
                    if (asset != null)
                        playableAssetObject = new SerializedObject(asset);
                }
            }
        }
        ...
        void DrawExtrapolationOptions()
        {
            ...
            if (hasPreExtrap)
            {
                ...
                EditorGUI.showMixedValue = m_PreExtrapolationTimeProperty.hasMultipleDifferentValues;
            ...
            // PostExtrapolation
            {
                ...
                EditorGUI.showMixedValue = m_PostExtrapolationTimeProperty.hasMultipleDifferentValues;
        ...
        void ShowPlayableAssetInspector()
        {
            ...
            if (m_SelectedPlayableAssetsInspector != null)
            {
                foreach (var selectedItem in m_SelectionCache)
                    PreparePlayableAsset(selectedItem);
        ...
        // Because what is animated is not the asset, but the instanced playable, we apply the animation clip here
        //  to preview what is being shown
        // This could be improved doing something more inline with animation mode, and reverting values that aren't
        //  be recorded later to avoid dirtying the asset
        static void PreparePlayableAsset(EditorClipSelection selectedItem)
        {
            ...
            // requires preview mode. reset the eval time so previous value is correct value is displayed
            //  while toggling
            if (!timelineWindow.state.previewMode)
            {
                selectedItem.lastEvalTime = -1;
                return;
            }

            double time = timelineWindow.state.editSequence.time;
            time = clip.ToLocalTime(time);

            // detect if the time has changed, or if the curves have changed
            if (selectedItem.lastEvalTime == time)
            {
                int curveVersion = AnimationClipCurveCache.Instance.GetCurveInfo(clip.curves).version;
                if (curveVersion == selectedItem.lastCurveVersion)
                    return;
                selectedItem.lastCurveVersion = curveVersion;
            }

            selectedItem.lastEvalTime = time;

            var clipInfo = AnimationClipCurveCache.Instance.GetCurveInfo(clip.curves);
            ...
            var prop = selectedItem.playableAssetObject.GetIterator();
            while (prop.NextVisible(true))
            {
                if (clip.IsParameterAnimated(prop.propertyPath))
                {
                    ...
                    switch (prop.propertyType)
                    {
                        ...
                        case SerializedPropertyType.Color:
                            SetAnimatedValue(clip, prop, "r", time);
                            SetAnimatedValue(clip, prop, "g", time);
                            SetAnimatedValue(clip, prop, "b", time);
                            SetAnimatedValue(clip, prop, "a", time);
                            break;
                        case SerializedPropertyType.Quaternion:
                        case SerializedPropertyType.Vector4:
                            SetAnimatedValue(clip, prop, "w", time);
                            goto case SerializedPropertyType.Vector3;
                        case SerializedPropertyType.Vector3:
                            SetAnimatedValue(clip, prop, "z", time);
                            goto case SerializedPropertyType.Vector2;
                        case SerializedPropertyType.Vector2:
                            SetAnimatedValue(clip, prop, "x", time);
                            SetAnimatedValue(clip, prop, "y", time);
                            break;
                    }
                }
            }
