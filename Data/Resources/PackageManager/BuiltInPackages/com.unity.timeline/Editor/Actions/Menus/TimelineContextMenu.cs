...
namespace UnityEditor.Timeline
{
    ...
    static class SequencerContextMenu
    {
        static class Styles
        {
            ...
            public static readonly string addItemWithSubMenuTemplate     = L10n.Tr("Add {0}/");
        ...
        public static void AddClipMenuCommands(GenericMenu menu, TrackAsset trackAsset, double candidateTime)
        {
            ...
            foreach (var assetType in assetTypes)
            {
                ...
                AddItemFromAssetCommands(menu, commandNameTemplate, assetType, onObjectChanged, !trackAsset.lockedInHierarchy);
            }
            ...
            foreach (var assetType in visibleAssetTypes)
            {
                ...
                AddCommandToMenu(menu, commandName, command, !trackAsset.lockedInHierarchy);
            }
        }

        public static void AddMarkerMenuCommands(GenericMenu menu, IEnumerable<Type> markerTypes, Func<Type, IMarker> addMarkerCommand, bool enabled)
        {
            foreach (var markerType in markerTypes)
            {
                var subMenuPrefix = string.Empty;
                if (TypeUtility.ObjectReferencesForType(markerType).Any())
                    subMenuPrefix = string.Format(Styles.addItemWithSubMenuTemplate, TypeUtility.GetDisplayName(markerType));

                var commandName = subMenuPrefix + string.Format(Styles.addItemTemplate, TypeUtility.GetDisplayName(markerType));
                ...
                AddItemFromAssetCommands(menu, subMenuPrefix + Styles.addItemFromAssetTemplate, markerType, onObjectChanged, enabled);
            }
        }
