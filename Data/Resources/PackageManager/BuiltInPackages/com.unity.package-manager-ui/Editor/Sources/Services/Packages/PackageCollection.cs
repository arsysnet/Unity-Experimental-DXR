...
namespace UnityEditor.PackageManager.UI
{
    ...
    internal class PackageCollection
    {
        ...
        private void RebuildPackageDictionary()
        {
            ...
            // Hide display packages with outdated package info and send a request for latest info
            foreach (var p in outdatedDisplayPackages)
            {
                packages.Remove(p.Name);
                FetchLatestPackageInfo(p, result => { UpdatePackageCollection(true); });
            }
        }
