...
- (A) The [Add package from disk button](#extpkg), which allows you to specify the location of an external package.
- (B) The [package scope drop-down menu](#scope), which allows you to filter which packages appear in the list.
- (C) The [Advanced button](#advanced), which allows you to display preview packages in addition to packages that have been verified to work with Unity.
- (D) The [search box](#searchText), which allows you to look for packages by name.
- (E) The [packages list view](#PackManLists), which displays all packages that match the filter and search parameters you specify.
- (F) The [package-specific detail view](#PackManDetails), which displays information specific to the package selected in the list.
- (G) The [status bar](#statusbar), which displays messages about the package load status and warnings about the network.
...
The following icons located to the right of the package name show you the status of the package:
...
![download icon](Images/PackageManagerUI-downloadicon.png) The download icon indicates that the installed packages have an [available update](#PackManUpdate).
...
To view the list of versions available for a package, click the expander icon to the left of the package name.

If there are updates available, they are displayed along with the **See all versions** option.

![Available updates list](Images/PackageManagerUI-AvailabeUpdates.png)

The  **See all versions** submenu displays all available versions.
...
![Available version list](Images/PackageManagerUI-AvailabeVersions.png)
...
These details include the following information:
 ...
 - (G) Package authors can include sample code in the package that you can import into your project. To import the sample code, click the **Import in project** button next to the sample.
 - (H) Button to [install](#PackManInstall) or [update](#PackManUpdate) a package (or the **Up to date** message if the selected version is already installed.)
 - (I) Button to [remove](#PackManRemove) or [disable](#PackManDisable) the package.
...
To access any of these pages, click the **View documentation**, **View changelog**, or **View licenses** links.
...
3. Select the version to install. For more information on available versions, see the section on the [List of versions](#VersionList).
...
| `category` | String |The category this package is in. For example, `"Forces"`. |
...
|Date|Reason|
|---|---|
|December 12, 2018| Documentation updated. Matches core package version.|
