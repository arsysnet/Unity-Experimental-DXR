...
half3 SHEvalLinearL0L1L2_SampleProbeVolume(half4 normal, float3 worldPos)
{
    normal.xyz = normalize(normal.xyz);
    ...
    float3 position = (transformToLocal == 1.0f) ? mul(unity_ProbeVolumeWorldToObject, float4(worldPos, 1.0)).xyz : worldPos;
    float3 texCoord = (position - unity_ProbeVolumeMin.xyz) * unity_ProbeVolumeSizeInv.xyz;
    const float shCoeffCount = 7.0f;
    texCoord.x = texCoord.x / shCoeffCount;

    // We need to compute proper X coordinate to sample.
    // Clamp the coordinate otherwize we'll have leaking between RGB coefficients
    float texCoordX = clamp(texCoord.x, 0.5f * texelSizeX, 1.0f / shCoeffCount - 0.5f * texelSizeX);

    texCoord.x = texCoordX;
    half4 SHAr = UNITY_SAMPLE_TEX3D_SAMPLER_LOD(unity_ProbeVolumeSH, unity_ProbeVolumeSH, texCoord, 0);

    texCoord.x = texCoordX + 1.0f / shCoeffCount;
    half4 SHAg = UNITY_SAMPLE_TEX3D_SAMPLER_LOD(unity_ProbeVolumeSH, unity_ProbeVolumeSH, texCoord, 0);

    texCoord.x = texCoordX + 2.0f / shCoeffCount;
    half4 SHAb = UNITY_SAMPLE_TEX3D_SAMPLER_LOD(unity_ProbeVolumeSH, unity_ProbeVolumeSH, texCoord, 0);

    // Linear + constant polynomial terms
    half3 x0;
    x0.r = dot(SHAr, normal);
    x0.g = dot(SHAg, normal);
    x0.b = dot(SHAb, normal);

    texCoord.x = texCoordX + 3.0f / shCoeffCount;
    half4 SHBr = UNITY_SAMPLE_TEX3D_SAMPLER_LOD(unity_ProbeVolumeSH, unity_ProbeVolumeSH, texCoord, 0);

    texCoord.x = texCoordX + 4.0f / shCoeffCount;
    half4 SHBg = UNITY_SAMPLE_TEX3D_SAMPLER_LOD(unity_ProbeVolumeSH, unity_ProbeVolumeSH, texCoord, 0);

    texCoord.x = texCoordX + 5.0f / shCoeffCount;
    half4 SHBb = UNITY_SAMPLE_TEX3D_SAMPLER_LOD(unity_ProbeVolumeSH, unity_ProbeVolumeSH, texCoord, 0);
    ...
    texCoord.x = texCoordX + 6.0f / shCoeffCount;
    half4 SHC = UNITY_SAMPLE_TEX3D_SAMPLER_LOD(unity_ProbeVolumeSH, unity_ProbeVolumeSH, texCoord, 0);
